...
keycloak_username = os.getenv("KEYCLOAK_USERNAME", "operator")
...
def post():
    pet = {"age": 0, "orgId": str(uuid.uuid4()), "kind": "string", "name": "string", "type": "DOMESTIC"}
    response = requests.post(f"{api_url}/api/operator/pets", json=pet, headers=auth_header)
...
def get():
    response = requests.get(f"{api_url}/api/operator/pets/{id}", headers=auth_header)
    response.raise_for_status()
    logger.debug(f"GET: {response.status_code} {response.json()}")


def get_all():
    response = requests.get(f"{api_url}/api/operator/pets", headers=auth_header)
    response.raise_for_status()
    logger.debug(f"GET ALL: {response.status_code}")


def patch():
    response = requests.patch(f"{api_url}/api/operator/pets/{id}", json={"age": 12}, headers=auth_header)
    response.raise_for_status()
    logger.debug(f"PATCH: {response.status_code} {response.json()}")


def delete():
    response = requests.delete(f"{api_url}/api/operator/pets/{id}", headers=auth_header)
    response.raise_for_status()
